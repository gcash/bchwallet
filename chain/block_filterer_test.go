			...
			TxIn: []*wire.TxIn{
				{
					PreviousOutPoint: wire.OutPoint{
						Hash: chainhash.Hash([32]byte{ // Make go vet happy.
							0xc4, 0x02, 0x97, 0xf7, 0x30, 0xdd, 0x7b, 0x5a, 0x99, 0x56,
							0x7e, 0xb8, 0xd2, 0x7b, 0x78, 0x75, 0x8f, 0x60, 0x75, 0x07,
							0xc5, 0x22, 0x92, 0xd0, 0x2d, 0x40, 0x31, 0x89, 0x5b, 0x52,
							0xf2, 0xff,
...
func TestBlockFiltererOneInOneOut(t *testing.T) {
	...
	watchedOutPoints[firstTx.TxIn[0].PreviousOutPoint] = &bchutil.AddressPubKeyHash{}
	watchedOutPoints[lastTx.TxIn[0].PreviousOutPoint] = &bchutil.AddressPubKeyHash{}
...
func TestOutOfOrderDependentTransaction(t *testing.T) {
	...
	// Watch for spend from prev in in first and last tx, both of which are
	// single input/single output.
	firstTx := Block200000.Transactions[0]
	lastTx := Block200000.Transactions[1]

	// Add each of their single previous outpoints to the set of watched
	// outpoints to filter for.
	internalAddrs := make(map[waddrmgr.ScopedIndex]bchutil.Address)
	internalAddrs[waddrmgr.ScopedIndex{Index: 0, Scope: waddrmgr.KeyScopeBIP0044}] = addr
	internalAddrs[waddrmgr.ScopedIndex{Index: 1, Scope: waddrmgr.KeyScopeBIP0044}] = addr2
	...
	// Filter block 200000, which should find matches for the watched
	// outpoints.
	match := blockFilterer.FilterBlock(&Block200000)
	if !match {
		t.Fatalf("failed to find matches when filtering for " +
			"1-in-1-out txns")
	}
	...
	// filterer, then we check that both the first and last txns are found
	// in that list.
	assertNumRelevantTxns(t, blockFilterer, 3)
	assertRelevantTxnsContains(t, blockFilterer, firstTx)
	assertRelevantTxnsContains(t, blockFilterer, lastTx)
}
...
func assertRelevantTxnsContains(t *testing.T, bf *chain.BlockFilterer, wantTx *wire.MsgTx) {
	...
	t.Fatalf("unable to find tx: %x in %d relevant txns", wantTx.TxHash(), len(bf.RelevantTxns))
}
