...
func TestOutputSplittingNotEnoughInputs(t *testing.T) {
	...
	output2Amount := bchutil.Amount(3)
...
func TestOutputSplittingOversizeTx(t *testing.T) {
	...
	if tx1.outputs[0].amount != bchutil.Amount(bigInput) {
	...
	if tx2.outputs[0].amount != bchutil.Amount(smallInput) {
...
func TestFulfillRequestsNotEnoughCreditsForAllRequests(t *testing.T) {
	...
	out2 := TstNewOutputRequest(
		t, 2, "3PbExiaztsSYgh6zeMswC49hLUwhTQ86XG", bchutil.Amount(2e6), net)
	out3 := TstNewOutputRequest(
		t, 3, "3Qt1EaKRD9g9FeL2DGkLLswhK1AKmmXFSe", bchutil.Amount(5e6), net)
...
func TestRollbackLastOutputMultipleInputsRolledBack(t *testing.T) {
	...
	for i, amount := range []bchutil.Amount{4, 3, 2} {
...
func TestRollBackLastOutputInsufficientOutputs(t *testing.T) {
	...
	output := &WithdrawalOutput{request: TstNewOutputRequest(
		t, 1, "34eVkREKgvvGASZW7hkgE2uNc1yycntMK6", bchutil.Amount(3), &chaincfg.MainNetParams)}
...
func TestRollbackLastOutputWhenNewOutputAdded(t *testing.T) {
	...
	checkTxChangeAmount(t, firstTx, bchutil.Amount(4))
	...
	checkTxChangeAmount(t, secondTx, bchutil.Amount(3))
}
...
func TestWithdrawalTxAddChange(t *testing.T) {
	...
	tx.calculateFee = TstConstantFee(bchutil.Amount(fee))
...
func TestWithdrawalTxInputTotal(t *testing.T) {
	...
	if tx.inputTotal() != bchutil.Amount(5) {
		t.Fatalf("Wrong total output; got %v, want %v", tx.outputTotal(), bchutil.Amount(5))
	}
}
...
func TestStoreTransactionsWithChangeOutput(t *testing.T) {
	...
	inputTotal := bchutil.Amount(0)
	for _, debit := range txDetails.Debits {
		inputTotal += debit.Amount
	}
	if inputTotal != bchutil.Amount(5e6) {
		t.Fatalf("Unexpected input amount; got %v, want %v", inputTotal, bchutil.Amount(5e6))
	}
...
func checkMsgTxOutputs(t *testing.T, msgtx *wire.MsgTx, requests []OutputRequest) {
	...
	for i, request := range requests {
		...
		gotAmount := bchutil.Amount(txOut.Value)
...
func checkTxChangeAmount(t *testing.T, tx *withdrawalTx, amount bchutil.Amount) {
	if !tx.hasChange() {
...
func checkLastOutputWasSplit(t *testing.T, w *withdrawal, tx *withdrawalTx,
	origAmount, newAmount bchutil.Amount) {
