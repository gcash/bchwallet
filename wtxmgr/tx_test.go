...
var (
	...
	TstRecvTx, _                    = bchutil.NewTxFromBytes(TstRecvSerializedTx)
	...
	TstSpendingTx, _           = bchutil.NewTxFromBytes(TstSpendingSerializedTx)
...
func serializeTx(tx *bchutil.Tx) []byte {
...
func TestInsertsCreditsDebitsRollbacks(t *testing.T) {
	...
	dupRecvTx, _ := bchutil.NewTxFromBytes(TstRecvSerializedTx)
	// Switch txout amount to 1 BCH.  Transaction store doesn't
	...
	TstDoubleSpendTx := bchutil.NewTx(newDupMsgTx)
	...
	spendingTxOut1 := wire.NewTxOut(1e7, []byte{5, 6, 7, 8, 9}, wire.TokenData{})
	spendingTxOut2 := wire.NewTxOut(9e7, []byte{10, 11, 12, 13, 14}, wire.TokenData{})
	spendingTx.AddTxOut(spendingTxOut1)
	spendingTx.AddTxOut(spendingTxOut2)
	TstSpendingTx := bchutil.NewTx(spendingTx)
	...
	tests := []struct {
		...
		},
		{
			...
			bal: 0,
			unc: bchutil.Amount(TstRecvTx.MsgTx().TxOut[0].Value),
			unspents: map[wire.OutPoint]struct{}{
				{
					Hash:  *TstRecvTx.Hash(),
					Index: 0,
		...
		},
		{
			...
			bal: bchutil.Amount(TstRecvTx.MsgTx().TxOut[0].Value),
			unc: 0,
			unspents: map[wire.OutPoint]struct{}{
				{
					Hash:  *TstRecvTx.Hash(),
		...
		},
		{
			...
			bal: bchutil.Amount(TstDoubleSpendTx.MsgTx().TxOut[0].Value),
			unc: 0,
			unspents: map[wire.OutPoint]struct{}{
				{
		...
		},
		{
			...
			bal: 0,
			unc: bchutil.Amount(TstSpendingTx.MsgTx().TxOut[0].Value),
			unspents: map[wire.OutPoint]struct{}{
				{
		...
		},
		{
			...
			bal: 0,
			unc: bchutil.Amount(TstSpendingTx.MsgTx().TxOut[0].Value + TstSpendingTx.MsgTx().TxOut[1].Value),
			unspents: map[wire.OutPoint]struct{}{
				{
					Hash:  *TstSpendingTx.Hash(),
					Index: 0,
...
func TestFindingSpentCredits(t *testing.T) {
	...
	expectedBal := bchutil.Amount(TstSpendingTx.MsgTx().TxOut[0].Value)
	if bal != expectedBal {
...
func TestCoinbases(t *testing.T) {
	...
	// Balance should be 0 if the coinbase is immature, 50 BCH at and beyond
	// maturity.
	//
...
func TestRemoveUnminedTx(t *testing.T) {
	...
	// balance.
	checkBalance := func(expectedBalance bchutil.Amount,
	...
	checkBalance(bchutil.Amount(initialBalance), false)
	checkBalance(bchutil.Amount(initialBalance), true)
	...
	checkBalance(bchutil.Amount(changeAmount), true)
