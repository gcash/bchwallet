...
// rescan request.
func (w *Wallet) activeData(dbtx walletdb.ReadTx) ([]bchutil.Address, []wtxmgr.Credit, error) {
...
func (w *Wallet) newAddress(addrmgrNs walletdb.ReadWriteBucket, account uint32,
	scope waddrmgr.KeyScope) (bchutil.Address, *waddrmgr.AccountProperties, error) {
	...
	err := walletdb.View(w.db, func(dbtx walletdb.ReadTx) error {
		...
		for i, txIn := range tx.TxIn {
			...
			vm, err := txscript.NewEngine(prevOutScript, tx, i,
				txscript.StandardVerifyFlags, nil, nil, nil, amount)
			if err == nil {
				err = vm.Execute()
			}
			if err != nil {
