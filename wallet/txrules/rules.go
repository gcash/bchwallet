...
func GetDustThreshold(scriptSize int, relayFeePerKb bchutil.Amount) bchutil.Amount {
	...
	relayFee := bchutil.Amount(totalSize) * byteFee
	return 3 * relayFee
}
...
func IsDustOutput(output *wire.TxOut, relayFeePerKb bchutil.Amount) bool {
	...
	return IsDustAmount(bchutil.Amount(output.Value), len(output.PkScript),
		relayFeePerKb)
}
...
func FeeForSerializeSize(relayFeePerKb bchutil.Amount, txSerializeSize int) bchutil.Amount {
	fee := relayFeePerKb * bchutil.Amount(txSerializeSize) / 1000

	if fee == 0 && relayFeePerKb > 0 {
		fee = relayFeePerKb
	}

	if fee < 0 || fee > bchutil.MaxSatoshi {
		fee = bchutil.MaxSatoshi
	}
