...
bchwallet will not drop transaction history by itself, as this is something that
...
```
$ cd $GOPATH/src/github.com/gcash/bchwallet/cmd/dropwtxmgr
$ go get
```
...
```
$ dropwtxmgr
Database path: /home/username/.bchwallet/mainnet/wallet.db
Drop all bchwallet transaction history? [y/N] y
...
```
$ dropwtxmgr --db ~/.bchwallet/testnet/wallet.db
Database path: /home/username/.bchwallet/testnet/wallet.db
Drop all bchwallet transaction history? [y/N] y
Dropping wtxmgr namespace
```

After dropping transaction history, bchwallet may be restarted and a full rescan
